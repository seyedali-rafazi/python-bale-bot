...
🔸 **بسته 5 گیگ** - پنجاه هزار تومن
🔸 **بسته 10 گیگ** - نود هزار تومن
🔸 **بسته 20 گیگ** - صد و هفتاد هزار تومن
🔸 **بسته 50 گیگ** - چهارصد هزار تومن
...
لطفاً بسته‌ای را انتخاب کنید:
    ...
    reply_markup = InlineKeyboardMarkup(keyboard)
    await query.edit_message_text(
        buy_cloud_text, reply_markup=reply_markup, parse_mode="Markdown"
    )
...
async def btn_cloud_files(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        reply_markup = InlineKeyboardMarkup(keyboard)
        await query.edit_message_text(
            files_text, reply_markup=reply_markup, parse_mode="Markdown"
        )
...
• حذف فایل، فضای ذخیره‌سازی را آزاد نمی‌کند
    ...
    keyboard = [
        [
            InlineKeyboardButton(
                "✅ تایید و پرداخت", callback_data=f"accept_cloud_purchase_{size_gb}"
            )
        ]
    ]
    reply_markup = InlineKeyboardMarkup(keyboard)
