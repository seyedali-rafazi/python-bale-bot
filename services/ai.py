...
async def ask_chatbot(text):
    ...
    async with chatbot_lock:
        ...
        try:
            ...
            prompt = (
                "این یک درخواست کاملاً مستقل است. به هیچ عنوان از پیام‌های قبلی به عنوان کانتکست استفاده نکن.\n"
                "به هیچ وجه نام من را در پاسخ نیاور، سلام و احوال‌پرسی نکن و مستقیماً فقط جواب سوال زیر را بده:\n"
                f"{text}"
            )
